...
constexpr int WIDTH_A4_LIST = 1500; // ширина листа в пикселях
constexpr int HEIGHT_A4_LIST = 1000; // высота листа в пикселях
constexpr int WIDTH_TABLE_LIST = 1300; // ширина таблицы в пикселях
constexpr int HEIGHT_TABLE_LIST = 700; // высота таблицы в пикселях
constexpr int TABLE_ROWS = 22;
constexpr int TABLE_COLS = 10;

/* открываем изображение в удобном размере*/
void resize_to_wsize(Mat& img) 
{
 resize(img, img, Size(2048, 1536)); 
}

/* нахождение углов листа на фотографии */
vector<Point> find_list_corners(Mat& img_)
{
 int BORDER_SIZE = 5; // толщина границы таблицы в пикселях
 
 adaptiveThreshold(img_, img_, 255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY, 115, 4); // адаптивное пороговое преобразование для элементов массива
 /* img_ — исходный массив(изображение), img_ — целевой массив
 ADAPTIVE_THRESH_GAUSSIAN_C — пиксели окрестности умножаются на весовой коэффициент (взвешиваются) в соответствии с гауссовой функцией 
 115 — весовой коэффициент, 4 — число пикселей в окрестности: */
 
 medianBlur(img_, img_, 11); // медианное сглаживание - сохраняет края, удаляя шум.
 /* img_ — исходный массив(изображение), img_ — целевой массив
 11 — линейный размер апертуры, насколько сильное сглаживание */
 
 copyMakeBorder(img_, img_, BORDER_SIZE, BORDER_SIZE, BORDER_SIZE, BORDER_SIZE, BORDER_CONSTANT, Scalar(0)); // граница вокруг изображения 
 /* img_ — исходный массив(изображение), img_ — целевой массив
 BORDER_SIZE верхняя , нижняя , левая , правая границы в пикселях
 BORDER_CONSTANT — добавляет постоянную цветную рамку, value — цвет границы (черный) */
 
 Canny(img_, img_, 100, 250); // детектор границ
 /* img_ — одноканальное изображение для обработки (градации серого), img_ — одноканальное изображение для хранения границ, найденных функцией
 100 — порог минимума, 250 — порог максимума */
 int max_contour_area = (img_.rows - BORDER_SIZE * 2) * (img_.cols - BORDER_SIZE * 2);
 int min_contour_area = (img_.rows / 2) * (img_.cols / 2);
 
 vector<vector<Point>> cont; // хранилище памяти для хранения данных найденных контуров
 vector<Vec4i> hierarchy;
 findContours(img_, cont, hierarchy, RETR_TREE, CHAIN_APPROX_SIMPLE);
 /* img_ — исходное изображение, cont — хранилище памяти для хранения данных найденных контуров
 hierarchy - иерархия вложенных контуров
 ...
 vector<Point> rcont;
 // отбираем контуры в виде прямоугольников
	for (int i = 0; i < cont.size(); ++i) 
	  {
    	auto lenc = arcLength(cont[i], true); // длина контура
		vector<Point2i> poly; // результат апроксимации
		approxPolyDP(cont[i], poly, 0.05 * lenc, true); // Аппроксимирует многоугольные кривые с заданной точностью.
		/*   cont[i] - входной вектор 2D-точки хранится в vector, poly - результат приближения. 
        0.05 * lenc - параметр, определяющий точность аппроксимации. 
        true - аппроксимируемая кривая замкнута (ее первая и последняя вершины связаны). */
		auto contr = contourArea(poly); // определяем вид апроксимируемого контура
		if (poly.size() == 4 && max_contour_area > contr && contr > min_contour_area) rcont = poly;
      }
 // определяем размер контура, убрав границы
...
/* структура для сортировки углов */
...
/* сортируем углы полученных контуров и запоминаем их координаты */
corners_sorted sort_corners(vector<Point> corners) 
{
 corners_sorted scors;
 if (corners.empty() || corners.size() != 4) return scors; // проверили, что наш контур прямоугольный
 sort(corners.begin(), corners.end(), [](const Point& a, const Point& b) -> bool {return(a.x < b.x); });
...
/* убираем перспективу */
Mat geo_resrore(Mat& src, corners_sorted cs, int widthr, int heightr) 
{
 vector<Point2f> cv = {	cs.top_left, cs.bot_left, cs.bot_right, cs.top_right };
 vector<Point2f> rv = {	Point2f((float)0,(float)0), Point2f((float)0,(float)heightr - 1), Point2f((float)widthr - 1, (float)heightr - 1), Point2f((float)widthr - 1, (float)0)	};

 Mat rmat;
 auto pt = getPerspectiveTransform(cv, rv);
 warpPerspective(src, rmat, pt, Size(widthr, heightr));
 return rmat;
}

/* суммарный пиксель */
int sum_cur_pix(const Mat& img, uchar pix) 
{
 int summ = 0;
...
/* находим окрестност маркера */
Point2f area_marker_find(const Mat& img, int marker_size) 
{
 vector<int> imctr(img.cols * img.rows, 0);
 int cpx, cpy = cpx = 0;
	for (int i = 0; i < img.rows - marker_size; ++i) 
	  {
		int bp = 0;
		cpx = 0;
		bp = sum_cur_pix(img(Rect(0, cpy, marker_size, marker_size)), 0);
		...
		for (int j = 0; j < img.cols - marker_size; ++j)
		  {
			int minus_pts = 0;
			for (int k = 0; k < marker_size; ++k) if (img.at<uchar>(i + k, j) == 0) minus_pts++;
			int plus_pts = 0;
			for (int k = 0; k < marker_size; ++k) if (img.at<uchar>(i + k, j + marker_size - 1) == 0) plus_pts++;
 ...
 auto mv = *(max_element(imctr.begin(), imctr.end()));
 ...
 return Point2f((float)px / cnt + padding, (float)py / cnt + padding);
}

/* находим левый верхний и правый нижний угол маркера используя ф-ию  area_marker_find*/
corners_sorted find_markers(Mat& img_) {
	adaptiveThreshold(img_, img_, 255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY, 55, 4);
	corners_sorted cs;
	
	cs.top_left = area_marker_find(img_(Rect2i(0, 0, 200, 500)), 37);

	cs.bot_left = area_marker_find(img_(Rect2i(0, 500, 200, 500)), 37);
	cs.bot_left.y += 500;

	cs.top_right = area_marker_find(img_(Rect2i(img_.cols - 201, 0, 200, 500)), 37);
	cs.top_right.x += img_.cols - 201;

	cs.bot_right = area_marker_find(img_(Rect2i(img_.cols - 201, 500, 200, 500)), 37);
...
/* удаляем тени */
void shadow_deleter_binariser(Mat& img) 
{
 auto k = getStructuringElement(MORPH_RECT, Size(5, 5));
 Mat img_;
 dilate(img, img_, k);
 medianBlur(img_, img_, 11);
 absdiff(img, img_, img);
	for (int i = 0; i < img.cols; ++i)
	  {
		for (int j = 0; j < img.rows; ++j)
		  {
 ...
 adaptiveThreshold(img, img, 255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY, 77, 7);
}

/* восстанавливаем линии */
int lineposrestore(const Mat& farea, int thresh) 
{
 vector<int> histy(farea.rows, 0);
 ...
 auto a = (int)(distance(histy.begin(), max_element(histy.begin(), histy.end())));
 return (a > thresh) ? a : -1;
}

/*удаляем шумы*/
void delete_noise(Mat& img, int kern, int yb, int ye) 
{
	 ...
	 for (int i = 0; i < img.cols - kern - 2; ++i) 
	   {
		...
		if (!hfind) 
		 {
		  rectangle(img, Rect(i + 1, j + 1, kern, kern), Scalar(255), -1);
		  i += kern + 1;
		 }
       }
	}
...
void ppl(Mat&& img, int& linepos, int& lineposx) 
{
 delete_noise(img, 2, 0, img.rows);
 int sm = 0;
 linepos = -1;
 lineposx = -1;
 vector<int> histy(img.rows, 0), histx(img.cols, 0);
...
int is_plus(cv::Mat&& img) 
{
	...
	for (int i = 0; i < img.rows; ++i) { //ñ÷èòàåì ãèñòó
		for (int j = 0; j < img.cols; ++j) {
